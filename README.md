...
# アプリ開発の背景
コロナ渦をきっかけとして、リモートワークが最近増えています。それにより自宅にいる時間も増えています。
普段の職場における労働と比べ、具体的には下記2点の傾向が増加しています。
・一人で部屋にこもりがち。
・仕事とプライベートの切り替えが曖昧。これまで仕事をしない時間帯や場所で仕事をする。
<br>

# 設定した問題定義
上記背景から普段の職場に比べ、メンタルヘルスが悪化する可能性があると考えました。
考えに至った主な理由は下記2点です。
・人との接点が減り、孤独を感じる。
...
# 目指した課題解決
上記問題を解決するため、それぞれ下記2つの課題を抽出しました。
・対面に変わる、孤独を感じない仕組みの確立
・清潔が保たれている空間の確保
<br>

# 洗い出した要件(課題解決手段)
抽出した上記2つの課題を解決すべく、7つの機能を抽出しました。
※現在(2020年11月時点)⑤までの機能を実装しています。

| **課題**                                                                                 | **課題解決手段**                                               | **具体的機能**                    |
| --------------------------------------------------------------------------------------- | ------------------------------------------------------------ | -------------------------------- |
| 対面に代わる、孤独を感じない仕組みの確立<br>⇨自分のアクションに対して他者からの<br>　リアクションがある | 整理整頓、清掃されたデスクの画像を<br>共有する。(自分のアクション)     | ① 画像投稿機能                    |
| ↑                                                                                       | ↑                                                            | ② 投稿画像の一覧表示機能            |
| ↑                                                                                       | 共有した画像にコメントを投稿できる。<br>(リアクション)               | ③ コメント投稿機能                 |
| 清潔が保たれている空間(デスクまわり)の確保                                                     | 清掃箇所の登録、定期的な提案                                     | ④ 清掃箇所及び関連情報の登録機能      |
| ↑                                                                                       | ↑                                                            | ⑤ アルゴリズムの実装、提案機能       |
| ↑                                                                                       | デスク上の異常(散らかっている状態)を検出する                        | ⑥ ラズベリーパイ+カメラによる監視機能 |
| ↑                                                                                       | デスク上の異常をアプリへ通知する。                                 | ⑦ 異常状態の通知機能               |

# 実装機能
**掃除登録及び提案機能**
登録した掃除場所情報を元に、最後の清掃した日からの経過日数をアプリ内で計算します。
その経過日数が登録した清掃期間を超えたものを今日の掃除箇所として提案します。
<img width="1000" src="https://user-images.githubusercontent.com/72062153/99951439-77fdec80-2dc1-11eb-8ad8-3e0f3caeb5c6.png">
提案された掃除箇所の「掃除完了」ボタンをクリックすると、該当箇所が非表示になります。
その後一定日数が経過し、再び清掃期間を超えると再度提案されます。
...
**画像投稿機能**
掃除した箇所の画像投稿及び共有ができます。投稿された画像にはコメントすることもできます。
...
# 実装予定の機能
・ラズベリーパイ及びカメラと連携し、部屋やデスク上の異常(散らかっている状態)検出機能
・検出された異常をアプリへ通知する機能
